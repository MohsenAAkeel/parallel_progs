        ...
        int    *errvalue)       /* return code for error, if any              */
{
    ...
    //printf("alloc_matrix called with r=%d,c=%d,e=%ld\n",nrows, ncols, element_size);
    ...
    *matrix = malloc (nrows * sizeof(void*));
    ...
    /* Step 3: Put the addresses into the pointers of the 2D matrix that 
       correspond to the starts of rows in the linear storage array. The 
       offset of each row in linear storage is a multiple of
      (ncols * element_size) bytes.  So we initialize
